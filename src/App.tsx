...
import { buildExport, canPickFolder, writeToPickedFolder, type ExportInput } from './lib/exporter'
...
export default function App() {
  ...
  const mixKey = tracks
    .map((t) => `${t.id}:${t.mute ? 1 : 0}${t.solo ? 1 : 0}:${t.gainDb}:${t.pan ?? panForRole(t.role)}`)
    .join('|')
  useEffect(() => {
    engine.setMix(
      tracks.map((t) => ({
  ...
  const onExport = useCallback(async () => {
    ...
    // Let the status paint before the synchronous render.
    ...
    try {
      const input: ExportInput = {
        ...
        region: p.region,
        tracks: p.tracks
          .filter((t) => t.status === 'ready' && t.audio)
          .map((t) => ({
            id: t.id,
            name: t.name,
            role: t.role,
      ...
      const stems = result.files.length - 1
      if (canPickFolder()) {
        setExportStatus('choose the folder that contains the take…')
        const outcome = await writeToPickedFolder(result)
        setExportStatus(
          outcome === 'written' ? `wrote ${result.folder}/ · ${stems} stems + sheet.txt` : 'export cancelled',
        )
      } else {
        const zip = buildZip(result.files.map((f) => ({ name: `${result.folder}/${f.name}`, data: f.data })))
        const url = URL.createObjectURL(new Blob([zip as BlobPart], { type: 'application/zip' }))
        const a = document.createElement('a')
        a.href = url
        a.download = `${result.folder}.zip`
        document.body.appendChild(a)
        a.click()
        a.remove()
        setTimeout(() => URL.revokeObjectURL(url), 10000)
        setExportStatus(`downloaded ${result.folder}.zip · ${stems} stems + sheet.txt`)
      }
    } catch (err) {
  ...
  const depths = new Set(
    ready.map((t) => (t.audio!.format === 'float' ? `${t.audio!.bitDepth}f` : `${t.audio!.bitDepth}`)),
  )
  const summary = [
