...
export default function App() {
  ...
  // ---- analysis -----------------------------------------------------------
  const runAnalysis = useCallback((overrides?: Record<string, boolean>) => {
    ...
    analyze(input).then(
      (result) => {
        if (projectRef.current !== null)
          dispatch({ type: "analysis-done", findings: result.findings });
      },
      (err: unknown) => {
  ...
  const fixedById = useMemo(() => {
    ...
    for (const t of ready) {
      ...
      out.set(t.id, [
        renderFixed(t.audio!.channels[0], fixes, t.audio!.sampleRate),
      ]);
    }
    return out;
  ...
  const mixKey = tracks
    .map(
      (t) =>
        `${t.id}:${t.mute ? 1 : 0}${t.solo ? 1 : 0}:${t.gainDb}:${t.pan ?? panForRole(t.role)}`,
    )
    .join("|");
  useEffect(() => {
    engine.setMix(
  ...
  const onRole = useCallback(
    (id: string, role: StemRole) => dispatch({ type: "set-role", id, role }),
    [],
  );
  const onMute = useCallback(
    (id: string) => dispatch({ type: "toggle-mute", id }),
    [],
  );
  const onSolo = useCallback(
    (id: string) => dispatch({ type: "toggle-solo", id }),
    [],
  );
  const onRegion = useCallback(
    (region: Region) => dispatch({ type: "set-region", region }),
    [],
  );
  const onVariant = useCallback(
    (v: Variant) => dispatch({ type: "set-variant", variant: v }),
    [],
  );
  ...
  const onGain = useCallback(
    (id: string, gainDb: number) => dispatch({ type: "set-gain", id, gainDb }),
    [],
  );
  const onPan = useCallback(
    (id: string, pan: number | null) => dispatch({ type: "set-pan", id, pan }),
    [],
  );
  const onMaster = useCallback(
    (db: number) => dispatch({ type: "set-master", masterDb: db }),
    [],
  );
  const onToggleMixer = useCallback(
    () => dispatch({ type: "toggle-mixer" }),
    [],
  );
  ...
  const summary = [
    ...
    rates.size === 1
      ...
      : rates.size > 1
        ...
        : null,
    depths.size === 1
      ? `${[...depths][0].replace("f", "-bit float")}${[...depths][0].endsWith("f") ? "" : "-bit"}`
      : depths.size > 1
        ? "mixed depths"
        : null,
    decoding > 0
      ? `decoding ${tracks.length - decoding}/${tracks.length}`
      : null,
    project && project.skipped > 0
      ? `${project.skipped} non-WAV ignored`
      : null,
  ]
    .filter(Boolean)
    .join(" · ");
      ...
      <header className="flex items-baseline gap-5 border-b border-rule bg-paper/80 px-8 py-4 backdrop-blur-sm">
        ...
        {project && (
          <>
            ...
            <span
              className={`font-mono text-xs ${rates.size > 1 ? "text-amber" : "text-muted"}`}
            >
              {summary}
            </span>
            <button
          ...
          tracks.length === 0 ? (
            <div className="flex flex-1 items-center justify-center bg-paper/80 p-16 text-center">
              <p className="font-display text-xl text-ink-soft">
                No WAV files in <span className="italic">{project.name}</span>.
                Drop another folder.
              </p>
            </div>
          ) : (
