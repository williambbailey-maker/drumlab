...
describe('fixes', () => {
  ...
  it('pad', () => {
    expect(Array.from(applyFix(x, { kind: 'pad', length: 6 }))).toEqual([0.1, 0.2, 0.3, 0.4, 0, 0].map((v) => Math.fround(v)))
  })
  it('flip', () => {
    expect(Array.from(applyFix(x, { kind: 'flip' }))).toEqual([-0.1, -0.2, -0.3, -0.4].map((v) => Math.fround(v)))
  })
  it('shift delays with zeros in front and keeps length', () => {
    expect(Array.from(applyFix(x, { kind: 'shift', samples: 2 }))).toEqual([0, 0, 0.1, 0.2].map((v) => Math.fround(v)))
    expect(Array.from(applyFix(x, { kind: 'shift', samples: -1 }))).toEqual([0.2, 0.3, 0.4, 0].map((v) => Math.fround(v)))
  })
  it('renderFixed applies in order and never mutates input', () => {
    const y = renderFixed(x, [{ kind: 'dc', offset: 0.1 }, { kind: 'flip' }, { kind: 'shift', samples: 1 }])
